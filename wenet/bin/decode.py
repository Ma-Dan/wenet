...
def main():
    ...
    feature_extractor = kaldifeat.Fbank(opts)

    wave, samp_freq = torchaudio.load(args.test_data)
    wave *= 32768
    wave = wave.squeeze()
    ...
    with torch.no_grad():
        ...
        feats = feature_extractor(wave).unsqueeze(0)
        feats = F.pad(feats, (0, 0, 0, 2000-feats.shape[1], 0, 0))
        print(feats.shape)
        feats_lengths = torch.Tensor([feats.shape[1]])
        ...
        logging.info('{}'.format(args.connect_symbol.join(content)))
